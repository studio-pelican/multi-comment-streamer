...
function OverlayContent() {
  ...
  return (
    <div className="w-full h-screen overflow-hidden p-6 flex flex-col justify-end items-start text-white bg-transparent">
      
      <div className="absolute top-4 left-4 flex flex-col gap-2 max-w-sm z-50">
      ...
      <div className="w-full flex flex-col justify-end items-start pb-4">
        <AnimatePresence initial={false}>
          {unifiedComments.map((comment) => (
            <motion.div
              key={comment.id}
              initial={{ opacity: 0, x: -20, scale: 0.95, height: 0, marginBottom: 0 }}
              ...
              className={`p-3 rounded-xl shadow-lg flex items-start gap-3 w-full max-w-sm backdrop-blur-sm origin-left ${
